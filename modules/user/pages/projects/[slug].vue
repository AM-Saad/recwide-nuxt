...
const { data, error } = await useLazyFetch(`/api/projects/${id}`, {
  headers,
...
const deleteProject = async (e: Event, id: string): Promise<void> => {
  e.preventDefault()
  await useLazyFetch(`/api/projects/${id}`, {
    method: "DELETE",
    headers,
  })
  router.push({ name: "projects" })
    ...
    <div class="wrapper my-4">
      <main v-if="rawProject">
          ...
          <div class="flex justify-between my-2">
            ...
            <div class="flex gap-2 items-center">
              <button
                ...
                @click="deleteProject($event, rawProject.id)"
              >
                Delete
              </button>

              <router-link
                :to="{ name: 'edit', params: { slug: rawProject.slug } }"
                class="btn btn-small"
              >
                Edit
              </router-link>
        ...
        <div class="grid md:grid-flow-col gap-4">
          ...
          >
            <video
              :src="config.public.SERVER_URL + i.url + '.mp4'"
              controls
              class="w-full h-full"
            >
            ...
            <div class="flex gap-2 m-2 mb-4">
              <a
                download
                :href="config.public.SERVER_URL + i.url + '.mp4'"
                class="btn btn-small bg-theme"
                >Download</a
              >
            </div>
          </div>
          <div v-if="rawProject.media.length === 0">No media found!</div>
        </div>
      </main>
    </div>
  </div>
