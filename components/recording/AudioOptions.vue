    ...
    [
      {
        ...
        needPermissions:
          !store.micGranted && store.audioSettings === AUDIO.MIC_AND_SYSTEM,
...
const changeSoundOpts = (mode: AUDIO): void => {
  store.recSettings(mode)
}
const checkKeyPressed = (e: KeyboardEvent, mode: AUDIO): void => {
  if (e.key === "Enter") {
...
<template>
  <div>
    ...
    >
      <div v-for="(opt, idx) in options" :key="opt.name">
        <div
          ...
          :tabindex="idx + 1"
          @click="changeSoundOpts(opt.name as AUDIO)"
          @keydown="checkKeyPressed($event, opt.name as AUDIO)"
        >
          <div class="flex items-center gap-x-2 dark:text-gray-100">
            <div
              class="icon h-6 w-6 stroke-1 dark:fill-slate-50 dark:stroke-slate-50"
              v-html="opt.icon"
            ></div>
            <span>{{ opt.name }}</span>
          </div>
    ...
    <div class="mt-5 grid gap-y-2">
      <label for="resolution" class="dark:text-gray-200">Resolution</label>
      <select
        id="resolution"
        v-model="currentResolution"
        class="glass-bg rounded-lg border p-2 text-gray-600"
        tabindex="5"
      >
        <option
          v-for="i in store.resolutions"
          :key="i.height"
          :value="i.height"
        >
          {{ i.height }}
